...
$pageCount = $pdf->setSourceFile('New-patient-registration-mn.pdf');
...
$results = $db->query('SELECT * FROM meridian_north_new_patients where ID =' . $ID . ' and AUTH = ' .$AUTH);
...
elseif ('Friend' == $row["SOURCE"]){
    	$pdf->SetXY(165.5, 239.2);
    	$pdf->Write(10, 'X');
}
elseif ('Previous' == $row["SOURCE"]){
    	$pdf->SetXY(198, 239.2);
    	$pdf->Write(10, 'X');
}
